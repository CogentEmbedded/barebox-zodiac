								  ...
								  const char *seat_location)
{
	...
	fd = rave_eeprom_dds_open(O_RDWR);
	if (fd < 0) {
	...
	if ((ret = rave_eeprom_dds_write_page(fd, RAVE_EEPROM_DDS_PAGE_FLAGS1, page)) != 0)
		goto out;
								  ...
								  const char *user_usb_power)
{
	...
	if ((ret = rave_eeprom_dds_write_page(fd, RAVE_EEPROM_DDS_PAGE_FLAGS3, page)) != 0)
		goto out;
