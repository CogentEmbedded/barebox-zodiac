...
static int do_rave_eeprom(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "ds:c:t:p:l:r:u:")) > 0) {
		switch (opt) {
		case 'd':
			dump = 1;
			break;
	...
	if (!dump && (dump_dds || dump_main)) {
		printf("no dump action given for EEPROM type\n");
		return COMMAND_ERROR_USAGE;
	}
	...
	if (dump)
		ret = rave_eeprom_dump (dump_main, dump_dds);
out:
...
BAREBOX_CMD_HELP_OPT("-s EEPROM\t", "select specific EEPROM (dds or main) when dumping contents")
...
BAREBOX_CMD_START(rave_eeprom)
	.cmd = do_rave_eeprom,
	BAREBOX_CMD_DESC("access RAVE EEPROM contents in human-readable format")
