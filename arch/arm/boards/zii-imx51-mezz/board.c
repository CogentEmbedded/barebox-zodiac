...
#define pr_fmt(fmt) "zii-mezz: " fmt
...
static void zii_mezz_power_init(struct mc13xxx *mc13xxx)
{
	u32 val;
...
static int zii_mezz_init(void)
{
	if (!of_machine_is_compatible("zii,imx51-mezz"))
		return 0;

	barebox_set_hostname("zii-mezz");

	mc13xxx_register_init_callback(zii_mezz_power_init);
...
coredevice_initcall(zii_mezz_init);
...
static int zii_mezz_late_init(void)
{
	int ret;
...
late_initcall(zii_mezz_late_init);

#ifdef CONFIG_ARCH_IMX_XLOAD

static int zii_mezz_xload_init_pinmux(void)
{
...
coredevice_initcall(zii_mezz_xload_init_pinmux);

static int zii_mezz_xload_init_devices(void)
{
...
device_initcall(zii_mezz_xload_init_devices);
