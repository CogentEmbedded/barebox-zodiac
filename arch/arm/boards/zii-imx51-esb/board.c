...
static void zii_esb_power_init(struct mc13xxx *mc13xxx)
{
	u32 val;
...
static int zii_esb_init(void)
{
	if (!of_machine_is_compatible("zii,imx51-esb"))
		return 0;

	barebox_set_hostname("zii-esb");

	mc13xxx_register_init_callback(zii_esb_power_init);
...
coredevice_initcall(zii_esb_init);
...
static int zii_esb_late_init(void)
{
	int ret;
	...
	return add_sndev_params();
}
late_initcall(zii_esb_late_init);

#ifdef CONFIG_ARCH_IMX_XLOAD

static int zii_esb_xload_init_pinmux(void)
{
...
coredevice_initcall(zii_esb_xload_init_pinmux);

static int zii_esb_xload_init_devices(void)
{
...
device_initcall(zii_esb_xload_init_devices);
