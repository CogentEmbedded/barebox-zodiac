...
static int mv88e6xxx_probe(struct device_d *dev)
{
	...
	/* internal MII bus first as ports may want to attach to phys on this bus */
	...
	if (mdio_node) {
		...
		priv->miibus.dev.device_node = mdio_node;
	...
	for_each_available_child_of_node(switch_node, port_node) {
		...
		phydev->dev.parent	= &priv->parent_miibus->dev;
	...
	if (/* TODO: check for eeprom access support in switch */
	    of_property_read_u32(dev->device_node, "eeprom-length", &eeprom_size)) {
		char *devname;
		const char *alias;
		...
		if (alias) {
			...
			if (err < 0) {
				dev_err(dev, "no index found to name EEPROM device\n");
				return err;
			}
